...
| 版本设置 → 版本号 | `V1.0.4`（格式 `V x.y.z`，每段是整数） |
| 游戏资料 → **隐私政策链接** | `https://luffryfight.github.io/liuyige/privacy.html`（开启步骤见第七之二节） |
| 商店 → 商店资料 | 走完基础信息 + 物料，再提交审核 |

版本号规则：新版本必须**高于**旧版本，且提审／发布的版本号不能与已上线或审核中的重复。用掉的编号统一记在 `tools/bump-version.cjs` 顶部（升档时会自己补上新号，不用在这里往下堆）；**本次传的是 V1.0.4，下次再传就得从 V1.0.5 起**。
...
都是这一刻的实际情况，写文案时照抄不会出问题：

- **六款收纳工作台皮肤**——栖湾原木台（默认）、玄铁九鼎案、赤霄朱漆台、青玉藏龙案、鎏金万象案、长夜星砂台；随整理进度与图鉴段数解锁，**纯外观，不影响摆放判定，也不算进整齐度**。
- **100 关全程免费，不看广告也能从头玩到尾。** 所以简介里别写「看广告解锁」之类的话：当前版本 `AD_UNIT_ID` 仍为空、一条广告也不发，写了就和 `docs/privacy.html` 里「没有接入任何广告 SDK」对不上（TapTap 口径是「政策须真实有效、与 App 内协议一致」）。等广告位真拿到手、文案同步改完再说。
...
皮肤那一组（`_verify-skins.cjs` 跑完留下的）里有**两张也达标**，想展示外观可以加进去：

| 文件 | 尺寸 / 比例 | 能不能传 |
|---|---|---|
| `skin-phone-panel.png` | 780×1688（0.462） | ✅ 手机上的皮肤面板，六款一览 |
| `skin-level45-iron.png` | 744×1405（0.529） | ✅ 同一关换皮肤前后对照 |
| `skin-{oak,iron,lacquer,jade,gold,star}.png` | 448×448（1:1） | ❌ 只有棋盘方块，尺寸不够 |
| `_contact.png` / `_c2.png` | 1330×470 等 | ❌ 横版对照图，内部复核用 |
...
- [ ] 图标／封面／宣传图／录屏／三份文案齐了
- [ ] 小游戏前置备案已启动
- [ ] 版本号填 `V1.0.4`（下次传从 V1.0.5 起，编号不能重复）
