...
产物信息（1.0.4）：包名 `com.liuyige.game`，versionCode `1004`，minSdk `24`，targetSdk `34`，约 113 KB。桌面图标下显示的应用名是 `留一格：回忆收纳师`，写在 `res/values/strings.xml` 的 `app_name` 里，须与 TapTap 商店页的应用名一致（别写成简称「留一格」）。
